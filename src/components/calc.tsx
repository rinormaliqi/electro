...
export default function SolarCalculatorSimplified() {
  const { t } = useTranslation();
  const DEFAULT_PRODUCTION_PER_KWP = 1400;
  const DEFAULT_ALL_TO_EUR = 0.01033;
  const KPW_PER_M2 = 0.18;
  const HOUSE_TIER_THRESHOLD_MONTHLY = 700;
  ...
  const [allToEur, setAllToEur] = useState<string>("0.01033");
  ...
  const allToEurNum = useMemo(() => parseSafeNumber(allToEur), [allToEur]);

  function householdEffectivePricePerKwh(monthlyConsumptionKwh: number) {
    return monthlyConsumptionKwh > HOUSE_TIER_THRESHOLD_MONTHLY ? 9.5 : 8.5;
  }

  function businessPricePerKwhALL(voltage: BusinessVoltage) {
    return voltage === "medium" ? 18.8 : 31.5;
  }
  ...
  const monthlyBillALL = useMemo(() => (currency === "ALL" ? monthlyBillNum : monthlyBillNum / allToEurNum), [monthlyBillNum, currency, allToEurNum]);

  const { monthlyConsumptionKwh, pricePerKwhALL } = useMemo(() => {
    if (consumerType === "business") {
      const price = businessPricePerKwhALL(businessVoltage);
      const consumption = price > 0 ? monthlyBillALL / price : 0;
      return { monthlyConsumptionKwh: consumption, pricePerKwhALL: price };
    } else {
      let price = 8.5;
      let consumption = price > 0 ? monthlyBillALL / price : 0;
      for (let i = 0; i < 5; i++) {
        const newPrice = householdEffectivePricePerKwh(consumption);
        if (Math.abs(newPrice - price) < 1e-6) break;
        price = newPrice;
        consumption = price > 0 ? monthlyBillALL / price : 0;
      }
      return { monthlyConsumptionKwh: consumption, pricePerKwhALL: price };
    }
  }, [consumerType, businessVoltage, monthlyBillALL]);

  const annualConsumptionKwh = useMemo(() => monthlyConsumptionKwh * 12, [monthlyConsumptionKwh]);
  const requiredKwp = useMemo(() => (productionPerKwpNum <= 0 ? 0 : annualConsumptionKwh / productionPerKwpNum), [annualConsumptionKwh, productionPerKwpNum]);
  ...
  const annualSavingsALL = useMemo(() => annualProductionKwh * pricePerKwhALL, [annualProductionKwh, pricePerKwhALL]);
  const annualSavingsEUR = useMemo(() => annualSavingsALL * allToEurNum, [annualSavingsALL, allToEurNum]);
  const grossCostEuro = useMemo(() => systemKwp * pricePerKwpEur, [systemKwp, pricePerKwpEur]);
  const grossCostALL = useMemo(() => grossCostEuro / allToEurNum, [grossCostEuro, allToEurNum]);
  const netCostEuro = useMemo(() => Math.max(0, grossCostEuro - grantEuroNum), [grossCostEuro, grantEuroNum]);
  const netCostALL = useMemo(() => netCostEuro / allToEurNum, [netCostEuro, allToEurNum]);
  const paybackYears = useMemo(() => (annualSavingsEUR <= 0 ? Infinity : netCostEuro / annualSavingsEUR), [netCostEuro, annualSavingsEUR]);
  ...
  const generatePDF = () => {
    ...
    doc.text('E&A GREEN SOLAR', 105, 15, { align: 'center' });
    doc.setFontSize(12);
    doc.text('RAPORT KURSIMESH SOLARE', 105, 25, { align: 'center' });
    ...
    y += 6;
    doc.text(`Monedha: ${currency}`, 14, y);
    y += 6;
    doc.text(`Fatura mujore: ${monthlyBill} ${currency}`, 14, y);
    y += 6;
    ...
    y += 6;
    doc.text(`Çmimi i energjisë: ${fmt(pricePerKwhALL)} ALL/kWh`, 14, y);
    y += 15;
    
    // Kolona e djathtë
    const rightColumnX = 110;
    let rightY = 80;
    doc.text(`Kursim mujor: ${currency === 'ALL' ? fmt(annualSavingsALL / 12) + ' ALL' : '€' + fmt(annualSavingsEUR / 12)}`, rightColumnX, rightY);
    rightY += 6;
    doc.text(`Kursim vjetor: ${currency === 'ALL' ? fmt(annualSavingsALL) + ' ALL' : '€' + fmt(annualSavingsEUR)}`, rightColumnX, rightY);
    rightY += 6;
    ...
    // Shfaq koston në ALL nëse valuta është ALL
    if (currency === 'ALL') {
      doc.text(`Kosto e sistemit: ${fmt(grossCostALL)} ALL`, 14, y);
      y += 6;
    }
    ...
    // Shfaq koston neto në ALL nëse valuta është ALL
    if (currency === 'ALL') {
      doc.text(`Kosto neto: ${fmt(netCostALL)} ALL`, 14, y);
      y += 6;
    }
    ...
    const notes = [
      ...
      '• Vlerësimet bazohen në çmimet aktuale dhe kushtet lokale në Shqipëri',
      '• Për një analizë të detajuar, kontaktoni ekspertët tanë',
      `• Telefon: +355 69 282 3346 | Email: eagreensolar@gmail.com`
    ];
    ...
    doc.text('E&A GREEN SOLAR - Zgjidhje të besueshme për energjinë diellore', 105, 290, { align: 'center' });
    
    doc.save(`Raport_Solar_E&A_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div id="printable-calculator" style={{ display: 'none' }}>
        <div className="print-container">
          <div className="print-header">
            <h1>E&A GREEN SOLAR</h1>
            <h2>RAPORT KURSIMESH SOLARE</h2>
          ...
          <div className="print-section">
            ...
            <p><strong>Monedha:</strong> {currency}</p>
            <p><strong>Fatura mujore:</strong> {monthlyBill} {currency}</p>
          ...
          <div className="print-results">
            <div className="print-section">
              ...
              <p><strong>Çmimi i energjisë:</strong> {fmt(pricePerKwhALL)} ALL/kWh</p>
            </div>

            <div className="print-section">
              <h3>Kursimet</h3>
              <p><strong>Kursim mujor:</strong> {currency === 'ALL' ? fmt(annualSavingsALL / 12) + ' ALL' : '€' + fmt(annualSavingsEUR / 12)}</p>
              <p><strong>Kursim vjetor:</strong> {currency === 'ALL' ? fmt(annualSavingsALL) + ' ALL' : '€' + fmt(annualSavingsEUR)}</p>
          ...
          <div className="print-section">
            ...
            {currency === 'ALL' && <p><strong>Kosto e sistemit:</strong> {fmt(grossCostALL)} ALL</p>}
            {grantEuroNum > 0 && <p><strong>Subvencion:</strong> - €{fmt(grantEuroNum)}</p>}
            <p><strong>Kosto neto:</strong> €{fmt(netCostEuro)}</p>
            {currency === 'ALL' && <p><strong>Kosto neto:</strong> {fmt(netCostALL)} ALL</p>}
          </div>

          <div className="print-section">
            <h3>Kontakt</h3>
            <p><strong>E&A GREEN SOLAR</strong></p>
            <p>📞 +355 69 282 3346</p>
            <p>✉️ eagreensolar@gmail.com</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            ...
            {/* Lloji i Konsumatorit dhe Monedha */}
            ...
            {consumerType === "business" && (
              <div className="mt-4">
                ...
                >
                  <option value="medium">{t('calculator.input.mediumVoltage')}</option>
                  <option value="high">{t('calculator.input.highVoltage')}</option>
                </select>
              </div>
            )}
            ...
            <div className="mt-6 space-y-4">
              ...
              {/* Fushat e avancuara - të fshehura fillimisht */}
              <details className="group">
                ...
                <div className="mt-3 space-y-4 pl-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('calculator.input.exchangeRate')}
                      <span className="text-xs text-gray-500 ml-1">{t('calculator.input.exchangeRateNote')}</span>
                    </label>
                    <input 
                      type="text" 
                      inputMode="decimal"
                      className="w-full rounded-xl border-2 border-gray-200 px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-colors"
                      value={allToEur} 
                      onChange={(e) => handleNumberInput(e.target.value, setAllToEur)} 
                    />
                  </div>
                </div>
              </details>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-lg p-6 border border-green-100">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <div className="text-2xl font-bold text-emerald-600">
                  {currency === "ALL" ? `${fmt(annualSavingsALL / 12)} ALL` : `€${fmt(annualSavingsEUR / 12)}`}
                </div>
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <div className="text-2xl font-bold text-emerald-600">
                  {currency === "ALL" ? `${fmt(annualSavingsALL)} ALL` : `€${fmt(annualSavingsEUR)}`}
                </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-200 mb-4">
              ...
              <div className="space-y-3">
                ...
                {currency === "ALL" && (
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600">{t('calculator.result.systemCostAll')}</span>
                    <span className="font-medium">{fmt(grossCostALL)} ALL</span>
                  </div>
                )}
                ...
                {currency === "ALL" && (
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="font-semibold text-gray-900">{t('calculator.result.netCostAll')}</span>
                    <span className="text-xl font-bold text-gray-900">{fmt(netCostALL)} ALL</span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                <div className="text-sm text-gray-600 space-y-1">
                  <div>{t('calculator.result.energyPriceUsed')}: <strong>{fmt(pricePerKwhALL)} ALL/kWh</strong></div>
...
>
  {t('calculator.cta.phone')}
              ...
              <a href="mailto:eagreensolar@gmail.com" className="bg-white text-green-600 px-6 py-3 rounded-xl font-medium hover:bg-gray-100 transition-colors">
